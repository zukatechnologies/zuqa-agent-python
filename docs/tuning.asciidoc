...
Using an APM solution comes with certain trade-offs, and the Python agent for ZUQA is no different.
...
We invested and continue to invest a lot of effort to keep the overhead of using ZUQA as low as possible.
...
You can configure the ZUQA agent to capture headers of both requests and responses (<<config-capture-headers,`capture_headers`>>),
