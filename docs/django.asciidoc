...
Getting ZUQA set up for your Django project is easy, and there are various ways you can tweak it to fit to your needs.
...
Set up the ZUQA agent in Django with these two steps:

1.  Add `zuqa.contrib.django` to `INSTALLED_APPS` in your settings:
...
[source,shell]
----
ZUQA_SERVICE_NAME=<SERVICE-NAME>
ZUQA_SECRET_TOKEN=<SECRET-TOKEN>
----
...
The list given should be a list of regular expressions which are matched against the transaction name as seen in the ZUQA user interface:

[source,python]
----
ZUQA['TRANSACTIONS_IGNORE_PATTERNS'] = ['^OPTIONS ', 'views.api.v2']
----
...
[source,python]
----
ZUQA['DJANGO_TRANSACTION_NAME_FROM_ROUTE'] = True
----
...
No errors or metrics will be logged to ZUQA.

However, if during debugging you would like to force logging of errors to ZUQA, then you can set `DEBUG` to `True` inside of the ZUQA
...
LOGGING = {
    ...
    'loggers': {
        ...
        # Log errors from the ZUQA module to the console (recommended)
        'zuqa.errors': {
            'level': 'ERROR',
            'handlers': ['console'],
            'propagate': False,
...
Note that `exc_info=True` adds the exception information to the data that gets sent to ZUQA.
...
ZUQA will automatically log errors from your celery tasks, and record performance data.

[float]
[[django-logging-http-404-not-found-errors]]
==== Logging "HTTP 404 Not Found" errors

By default, ZUQA does not log HTTP 404 errors. If you wish to log
these errors, add
`'zuqa.contrib.django.middleware.Catch404Middleware'` to
...
To prevent the agent from sending any data to the APM Server during tests, set the `ZUQA_DISABLE_SEND` environment variable to `true`, e.g.:

[source,python]
----
ZUQA_DISABLE_SEND=true python manage.py test
----
...
ZUQA comes with a Django command that helps troubleshooting your setup. To check your configuration, run
