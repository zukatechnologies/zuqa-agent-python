...
Getting ZUQA set up for your Flask project is easy,
...
apm = ZUQA(app, service_name='<APP-ID>', secret_token='<SECRET-TOKEN>')
----
...
set the value of `DEBUG` in the `ZUQA` dictionary to `True`:
...
a couple of shortcuts are provided on the ZUQA Flask middleware object
...
Passing `logging=LEVEL` to the ZUQA constructor will make the agent automatically log all log messages from Python's built-in `logging` module,
...
apm = ZUQA(app, logging=logging.ERROR)
----

To log all messages independent of log levels to ZUQA, pass `logging=True` to the constructor:

[source,python]
----
from zuqa.contrib.flask import ZUQA

apm = ZUQA(app, logging=True)
----

NOTE: using `logging=True` can lead to high number of logs being sent to ZUQA.
We recommend to always limit logging with ZUQA to a high level (`WARNING` or `ERROR`).
...
The ZUQA agent will automatically send errors and performance data from your Celery tasks to the APM Server.
...
The context processor is installed automatically when you initialize `ZUQA`.
