...
`zuqa_labels`, using the official ECS https://www.elastic.co/guide/en/ecs/current/ecs-tracing.html[tracing fields].
...
----
import logging
from zuqa.handlers.logging import LoggingFilter
...
The ZUQA Python agent uses logging to log internal events and issues.
By default, it will use a `logging` logger.
If your project uses structlog, you can tell the agent to use a structlog logger
by setting the environment variable `ZUQA_USE_STRUCTLOG` to `true`.
...
ZUQA Python Agent, your logs must contain one or more of the following
identifiers:
...
recommend first making sure your LogRecord objects have the zuqa
...
----
import logging
from zuqa.handlers.logging import Formatter
  ...
  "processors": [
    {
      "grok": {
        "field": "message",
        "patterns": ["%{GREEDYDATA:msg} | zuqa transaction.id=%{DATA:transaction.id} trace.id=%{DATA:trace.id} span.id=%{DATA:span.id}"]
      }
    }
  ]
}
